...
ggplot(df, aes(age, mins_percentage)) +
  ...
  #geom_point(data=subset(team_mins, signing==1), size=1, color='#006600', alpha=0.35) +
  #geom_point(data=subset(team_mins, sold==1), size=1, color='#CC3333', alpha=0.35) +
  scale_x_continuous(breaks = c(18, 20, 22, 24, 26, 28, 30, 32, 34, 36, 38, 40))+
  geom_text_repel(data=df, aes(label=player), size=1.8, fontface="bold", segment.alpha = 0.6, segment.color = 'grey50', color='#333333')+
  annotate("text", x = 26.5, y = 0.05, color='#bc200b', size=2.2, label = "bold('Peak Years')", parse=TRUE)+
  #geom_text_repel(data=subset(team_mins, signing==1), aes(label=player), size=1.8, fontface="bold", segment.alpha = 0.6, segment.color = 'grey50', color='#006600', alpha=0.35)+
  #geom_text_repel(data=subset(team_mins, sold==1), aes(label=player), size=1.8, fontface="bold", segment.alpha = 0.6, segment.color = 'grey50', color='#CC3333', alpha=0.35)+
  scale_y_continuous(labels = function(x) paste0(x*100, "%"), breaks=seq(0, 1, 0.25))+
  ggtitle("Atletico Age-Utility Matrix")+
  labs(x="Age", y="Percentage of Mins Played", subtitle="La Liga 18/19")
